...
namespace Stock.API.Consumers
{
    public class OrderCreatedEventConsumer : IConsumer<OrderCreatedEvent>
    {
        ...
        public async Task Consume(ConsumeContext<OrderCreatedEvent> context)
        {
            ...
            {
                stockResult.Add(
                    (await _stockCollection.FindAsync(s => s.ProductId.ToString() == orderItem.ProductId && 
            ...
            if(stockResult.TrueForAll(sr => sr.Equals(true)))
            {
                foreach(OrderItemMessage orderItem in context.Message.OrderItems)
                {
                  Stock.API.Models.Entities.Stock stock = 
                        await (await _stockCollection.FindAsync(s => s.ProductId.ToString() == orderItem.ProductId))
                        .FirstOrDefaultAsync();

                    stock.Count -= orderItem.Count;
                    await _stockCollection.FindOneAndReplaceAsync(s => s.ProductId.ToString() == orderItem.ProductId, stock);
                }
